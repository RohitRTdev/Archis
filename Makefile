...
QEMU_CPU_ARGS_WITH_ACCEL := -M q35,accel=kvm -cpu host -smp sockets=1,cores=3,threads=2
QEMU_CPU_ARGS_WITHOUT_ACCEL := -smp sockets=1,cores=3,threads=2 -cpu Skylake-Client,+smap,+smep,+umip,+pge
...
test:
	...
	-drive file=$(OUTPUT_DIR)/archis_os.img,format=raw,if=none,id=drv0 \
	-device virtio-blk-pci,drive=drv0,disable-legacy=on \
	-m 512M -serial stdio | tee >(sed 's/\x1b\[[0-9;=]*[A-Za-z]//g' > $(OUTPUT_DIR)/con_log.txt)
